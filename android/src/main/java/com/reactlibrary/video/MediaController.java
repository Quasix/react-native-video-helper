...
public class MediaController {
  ...
  public native static int convertVideoFrame(ByteBuffer src, ByteBuffer dest, int destFormat,
      int width, int height, int padding, int swap);
  ...
  private long readAndWriteTrack(MediaExtractor extractor, MP4Builder mediaMuxer,
      MediaCodec.BufferInfo info, long start, long end, File file, boolean isAudio)
      throws Exception {
    ...
    if (resultWidth != 0 && resultHeight != 0) {
      ...
      try {
        ...
        if (resultWidth != originalWidth || resultHeight != originalHeight) {
          ...
          if (videoIndex >= 0) {
            ...
            try {
              ...
              if (Build.VERSION.SDK_INT < 18) {
                ...
                Log.e("tmessages",
                    "codec = " + codecInfo.getName() + " manufacturer = " + manufacturer
                        + "device = " + Build.MODEL);
              } else {
                      ...
                      if (doRender) {
                        ...
                        if (!errorWait) {
                          if (Build.VERSION.SDK_INT >= 18) {
                            ...
                            if (listener != null) {
                              listener.onProgress(
                                  (float) info.presentationTimeUs / (float) duration * 100);
                            }
          ...
          long videoTime = readAndWriteTrack(extractor, mediaMuxer, info, startTime, endTime,
              cacheFile, false);
          if (videoTime != -1) {
            videoStartTime = videoTime;
          }
